...
void CTextures::init()
{
	...
	unsigned char const screen[4] = { 0, 255, 0, 255 };
	Textures.greenScreen = gInts.Surface->CreateNewTextureID(true);
	gInts.Surface->DrawSetTextureRGBA(Textures.greenScreen, screen, 1, 1);
}
...
void CDrawManagerEx::RoundRect(int x, int y, int w, int h, int radius, int texture)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			float _a = DEG2RAD1(a + j * 6.f);

			round[(i * 16) + j] = Vertex_t(Vector2(_x + radius * sin(_a), _y - radius * cos(_a)));
		}
	}
