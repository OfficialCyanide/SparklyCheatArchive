...
public:
	...
	void InitKeyValues(KeyValues* kv, char* psz)
	{
		typedef int(__thiscall *_InitKeyValues)(KeyValues*, const char*);
		DWORD ptr = gSignatures.GetEngineSignature("55 89 E5 56 83 EC 14 E8 ? ? ? ? 58 8B 4D 0C 8B 75 08 C7 06");

		_InitKeyValues InitKeyVals = (_InitKeyValues)ptr;
		InitKeyVals(kv, psz);
	}

	bool LoadFromBuffer(KeyValues* kv, const char* resourceName, const char* pBuffer, void* pFileSystem = NULL, const char* pPathID = NULL)
	{
		typedef bool(__thiscall *_LoadFromBuffer)(KeyValues*, const char*, const char*, void*, const char*);

		static _LoadFromBuffer LoadFromBufferFn = 0;
		static bool SearchFunction = false;

		DWORD dwFunctionAddress = gSignatures.GetEngineSignature("55 89 E5 53 57 56 83 EC 7C E8 ? ? ? ? 58 89 45 A0");
		//DWORD dwFunctionAddress = gSignatures.GetEngineSignature("55 89 E5 53 57 56 83 EC 5C E8 ? ? ? ? 5A 8B 5D 10");

		if (dwFunctionAddress)
			LoadFromBufferFn = (_LoadFromBuffer)dwFunctionAddress, SearchFunction = true;

		if (LoadFromBufferFn && SearchFunction)
			return LoadFromBufferFn(kv, resourceName, pBuffer, pFileSystem, pPathID);
