...
void WindowStyle::About(int x, int y, int w, int h)
{
	...
	gBase.Surface->GetTextSize(gFonts.arial, L"Sparkly Rewrite is a free software. If you paid for this build, you were scammed.", iWidth, iHeight);
	gDraw.DrawString(CenterX - (iWidth / 2), y + 5, SColor(160), L"Sparkly Rewrite is a free software. If you paid for this build, you were scammed.");

	/*gBase.Surface->GetTextSize(gFonts.verdana, L"Have a question, want to chat, need to contact to me?", iWidth, iHeight);
	gBase.Surface->GetTextSize(gFonts.calibri_light, L"Join my Discord! www.Discord.io/SparklyRewrite", dcWidth, dcHeight);

	gDraw.DrawString(CenterX - (iWidth / 2), y + 100, SColor(200), L"Have a question, want to chat, need to contact to me?", gFonts.verdana);
	gDraw.DrawString(CenterX - (dcWidth / 2), y + 110 + iHeight, SColor(255), L"Join my Discord! www.Discord.io/SparklyRewrite", gFonts.calibri_light);*/

	gBase.Surface->GetTextSize(gFonts.verdana, L"I also have some videos with info and updates on Sparkly!", iWidth, iHeight);
	gBase.Surface->GetTextSize(gFonts.calibri_light, L"Watch my channel! www.YouTube.com/c/HoldonYT", dcWidth, dcHeight);

	gDraw.DrawString(CenterX - (iWidth / 2), y + /*200*/ 150, SColor(200), L"I also have some videos with info and updates on Sparkly!", gFonts.verdana);
	gDraw.DrawString(CenterX - (dcWidth / 2), y + /*210*/ 160 + iHeight, SColor(255), L"Watch my channel! www.YouTube.com/c/HoldonYT", gFonts.calibri_light);
}