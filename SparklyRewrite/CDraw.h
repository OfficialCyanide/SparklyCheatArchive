...
public:
	...
	TextureHolder(const byte* pRawRGBAData, int W, int H)
	{
		...
		if (!m_iTexture)
			return;
		gBase.Surface->DrawSetTextureRGBA(m_iTexture, pRawRGBAData, W, H);
	...
	{
		if (!gBase.Surface->IsTextureIDValid(m_iTexture))
			return false;
		gBase.Surface->DrawSetColor(clr[0], clr[1], clr[2], clr[3]);
		gBase.Surface->DrawSetTexture(m_iTexture);
		gBase.Surface->DrawTexturedRect(x, y, x + (m_iW * scale), y + (m_iH * scale));
		return true;
	};
